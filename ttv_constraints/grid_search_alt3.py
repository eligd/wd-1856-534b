...
def loop_body(i):
    ...
    shape = (p2_eccentricity_points, p2_inclination_points, p2_mean_anomaly_points, p2_argument_points)
    eccentricity_idx, inclination_idx, mean_anomaly_idx, argument_idx = np.unravel_index(i, shape)

    theta = [p2_mass, p2_period, p2_eccentricities[eccentricity_idx], p2_inclinations[inclination_idx], 
    ...
    t_p = np.array(t_p)
    R = romer_delay(p1_period, p2_period, p1_mass, p2_mass, stellar_mass, p1_inclination, ref_transit, t_p)
    exp_ttv += R
...
p2_mass = config['p2_mass']
p2_period = config['p2_period']

p2_eccentricity_min = config['p2_eccentricity_min']
p2_eccentricity_max = config['p2_eccentricity_max']
p2_eccentricity_points = config['p2_eccentricity_points']
p2_eccentricities = np.linspace(p2_eccentricity_min, p2_eccentricity_max, p2_eccentricity_points)

p2_inclination_min = config['p2_inclination_min']
p2_inclination_max = config['p2_inclination_max']
p2_inclination_points = config['p2_inclination_points']
p2_inclinations = np.linspace(p2_inclination_min, p2_inclination_max, p2_inclination_points)
...
if __name__ == '__main__':
    ...
    grid_params = np.meshgrid(p2_eccentricities, p2_inclinations, p2_mean_anomalies, indexing='ij')
    chi2_arr = []
    bar_format = '{l_bar}{bar:20}{r_bar}{bar:-10b}'
    N = p2_eccentricity_points*p2_inclination_points*p2_mean_anomaly_points*p2_argument_points
    chi2_arr = np.zeros((p2_eccentricity_points, p2_inclination_points, p2_mean_anomaly_points, p2_argument_points))
    ...
    chi2_arr = np.array(chi2_flattened).reshape((p2_eccentricity_points, p2_inclination_points, p2_mean_anomaly_points, p2_argument_points))
